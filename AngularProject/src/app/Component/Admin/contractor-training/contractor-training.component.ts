...
@Component({
  selector: 'app-contractor-training',
  templateUrl: './contractor-training.component.html',
  styleUrls: ['./contractor-training.component.scss'],
})
export class ContractorTrainingComponent implements OnInit {
  modalRef: BsModalRef;
  ContractorList: any = [];
  objContractor: any = {};
  Countries: any[];
  ...
  ProgramYear: any = [];
  userrole = JSON.parse(localStorage.getItem('currentUser') || '{}')
    .userRoleName;
  IsDisplay: any;
  contra: any;
  age: any;
  ...
  IsShow: any;
  IsRanking: any;
  ltcMemberId = JSON.parse(localStorage.getItem('currentUser') || '{}')
    .currentMemberId;
  ...
  ngOnInit(): void {
    ...
    if (this.userrole == 'LtcUser' || this.userrole == 'Admin')
      this.IsShow = true;
    ...
    this.DobYear = this.commonService.GetYearRange(
      1900,
      new Date().getFullYear(),
      'desc'
    );
    this.DateOfIssueYear = this.commonService.GetYearRange(
      1900,
      new Date().getFullYear(),
      'desc'
    );
    this.DateOfExpiryYear = this.commonService.GetYearRange(
      new Date().getFullYear(),
      2100
    );
    this.EducationYear = this.commonService.GetYearRange(
      1900,
      new Date().getFullYear(),
      'desc'
    );
    this.EmploymentYear = this.commonService.GetYearRange(
      1900,
      new Date().getFullYear(),
      'desc'
    );
  ...
  OtherLanguageCheckChange(index: any) {
    this.OtherLanguagelist[index].isChecked =
      this.OtherLanguagelist[index].isChecked == true
        ? !this.OtherLanguagelist[index].isChecked
        : true;
    this.objContractor.OtherLanguage = Array.prototype.map
      .call(
        this.OtherLanguagelist.filter(
          (r: { isChecked: boolean }) => r.isChecked == true
        ),
        (s) => ' ' + s.languageName
      )
      .toString()
  ...
  GetSortedContractorTraining() {
    this.contractorTrainingService.GetSortedContractorTraining().subscribe(
      (response: any) => {
        ...
        _.each(CandidateList, (Candidate: any) => {
          var rank: any = [];
          var TotalRank = 0;
          var Rn = RankingList.find(
            (x: any) =>
              x.ltcMemberId == this.ltcMemberId && x.candidateId == Candidate.id
          );
          if (Rn) {
            Candidate.LtcRank = Rn.rank;
          }
          ...
          var Total = 0;
          var Ranks = this.commonService.GetRanking(Candidate);
          _.each(Ranks, (item: any) => {
            if (item.value) Total += item.value;
          });
          Candidate.TotalRank = Total;
        });
  ...
  //Get All Language
  GetAllOtherLanguage() {
    this.isaService.GetAllOtherLanguage().subscribe(
      (response: any) => {
  ...
  GetAllContractorMaster() {
    this.contractorMasterService.GetAllContractorMaster().subscribe(
      (response: any) => {
  ...
  GetAllProgramMaster() {
    this.programMasterService.GetAllProgramMaster().subscribe(
      (response: any) => {
  ...
  GetAllCountries() {
    this.commonService.GetAllCountries().subscribe(
      (response: any) => {
        ...
        this.GetAllCountries = response;
        this.CountryOfNationalityList = response;
        
        this.ClassificationList = this.Countries.map(
          (item) => item.countryClass
  ...
  GetAllContractorTraining() {
    ...
    this.contractorTrainingService.GetAllContractorTraining(1).subscribe(
      (response: any) => {
  ...
  btnSaveRank(contra: NgForm) {
    ...
    this.contractorTrainingService
      .SaveRank(this.SaveRank)
      .subscribe((response: any) => {
        this.GetSortedContractorTraining();
        this.commonService.toastSuccessMsg(
          'Success',
          'Rank has been saved successfully'
        );
        this.closeModel();
      });
  }
  ...
  GetContractorTrainingById() {
    this.contractorTrainingService
      .GetContractorTraining(this.objContractor.id)
      .subscribe(
        (response: any) => {
          this.objContractor = response;
        },
  ...
  btnEdit(ContractorTr: any, Editmodal: TemplateRef<any>) {
    this.contractorTrainingService
      .GetSingleContractorTraining(ContractorTr.id)
      .subscribe(
        (response: any) => {
          this.objContractor = response;
          this.objContractor.dateofBirth = this.commonService.StringToDate(
            this.objContractor.dateofBirth
          );
          this.objContractor.dateofIssue = this.commonService.StringToDate(
            this.objContractor.dateofIssue
          );
          this.objContractor.dateofExpiry = this.commonService.StringToDate(
            this.objContractor.dateofExpiry
          );
          this.objContractor.PassportPhotopath =
            urlConstant.ContractorTraining.PassportPhotoPath +
            '' +
            this.objContractor.id +
            '/' +
            ContractorTr.passportPhotoName;
          if (this.objContractor.passportPhotoName == null) {
            this.objContractor.PassportPhotopath = null;
          }
          this.objContractor.employment.forEach((obj: any) => {
          ...
          if (this.objContractor.qualification) {
            ...
            array.forEach((obj: string) => {
              ...
              if (index != -1) {
                this.objContractor.qualificationList.push(
                  this.QualificationList[index]
                );
              } else {
          ...
          if (this.objContractor.otherLanguage != null) {
            ...
            for (let i = 0; i < res.length; i++) {
              this.OtherLanguagelist.filter(
                (x) => x.languageName == res[i].trim()
              ).map((x) => (x.isChecked = true));
            }
          }
  ...
  btnUpdate(contra: NgForm) {
    ...
    if (this.objContractor.id != null) {
      this.objContractor.dateofBirth = this.commonService.DateToString(
        this.objContractor.dateofBirth
      );
      this.objContractor.dateofIssue = this.commonService.DateToString(
        this.objContractor.dateofIssue
      );
      this.objContractor.dateofExpiry = this.commonService.DateToString(
        this.objContractor.dateofExpiry
      );

      this.objContractor.employment.forEach((obj: any) => {
        obj.fromMonth =
          this.Months.indexOf(obj.fromMonth) > 0
            ? this.Months.indexOf(obj.fromMonth)
            : null;
        obj.toMonth =
          this.Months.indexOf(obj.toMonth) > 0
            ? this.Months.indexOf(obj.toMonth)
            : null;
      });

      if (this.objContractor.qualificationList)
        this.objContractor.qualification = [];
      this.objContractor.qualification = this.objContractor.qualificationList
        .concat(
          this.ExtraQualificationList.map(function (obj: {
            extraQualification: any;
          }) {
            return obj.extraQualification;
          })
        )
        .join(', ');

      this.contractorTrainingService
        .UpdateContractorTraining(this.objContractor)
        .subscribe((response: any) => {
          if (response) {
  ...
  //View-Detail
  openModalWithClass(template: TemplateRef<any>, constructor: any, flag: any) {

    //var link = "admin/ViewApplication/" + constructor.id;
    //this.router.navigate(["admin/ViewApplication", constructor.id]);
    const link = `admin/ViewApplication/${constructor.id}?flag=${flag}`;
    this.router.navigate([], { queryParams: { flag: null } }).then(() => {
      window.open(link, '_blank');
    });
    //this.router.navigate([]).then(result => { window.open(link, '_blank'); });

    // this.contractorTrainingService
    //   .GetSingleContractorTraining(constructor.id)
    //   .subscribe(
    //     (response: any) => {
    //       this.modalRef = this.BsModalService.show(
    //         template,
    ...
    //       this.age = this.commonService.getAge(this.objContractor.dateofBirth);
    //       if (
    //         this.ContractorMasterList &&
    //         this.objContractor.contractorId != 'Select' &&
    //         this.objContractor.contractorId
    //       ) {
    //         if (
    //           this.ContractorMasterList.filter(
    //             (r) => r.id == this.objContractor.contractorId
    //           )[0]
    //         ) {
    //           this.objContractor.contractorName =
    //             this.ContractorMasterList.filter(
    //               (r) => r.id == this.objContractor.contractorId
    //             )[0].contractorName;
    //         }
    ...
    //         this.objContractor.programApplyingId != 'Select' &&
    //         this.objContractor.programApplyingId
    //       )
    //         this.objContractor.programApplying = this.ProgramMasterList.filter(
    //           (x) => x.id == this.objContractor.programApplyingId
    //         )[0].program;
    ...
    //           if (
    //             this.ProgramMasterList &&
    //             obj.whichprogramme != 'Select' &&
    //             obj.whichprogramme
    //           )
    //             obj.whichprogrammeName = this.ProgramMasterList.filter(
    //               (r) => r.id == obj.whichprogramme
    //             )[0].program;
    //         });
    //       }
    //     },
  ...
  GetDaysForBirth() {
    let numberOfDays = new Date(
      this.objContractor.DateofBirthYear,
      this.Months.indexOf(this.objContractor.DateofBirthMonth) + 1,
      0
    ).getDate();
  ...
  GetDaysDateExpiry() {
    let numberOfDays = new Date(
      this.objContractor.DateofExpiryYear,
      this.Months.indexOf(this.objContractor.DateofExpiryMonth) + 1,
      0
    ).getDate();
    for (let i = 1; i <= numberOfDays; i++) {
  ...
  GetDaysDateIssue() {
    let numberOfDays = new Date(
      this.objContractor.DateofIssueYear,
      this.Months.indexOf(this.objContractor.DateofIssueMonth) + 1,
      0
    ).getDate();
    for (let i = 1; i <= numberOfDays; i++) {
  ...
  onPassportPhotoUpload(event: any) {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      if (ext == 'jpg' || ext == 'png' || ext == 'jpeg') {
        ...
        reader.onload = (e: any) => {
          this.objContractor.PassportPhotobase64 =
  ...
  DownloadFile(fileName: string, fileURL: any, FileExt: string) {
    ...
    if (isIEOrEdge) {
      var _window = window.open(
        urlConstant.ContractorTraining.Documents + FinalUrl,
        '_blank'
      );
      if (_window != null) {
      ...
      if (FileExt == 'doc' || FileExt == 'docx') {
        if (_window != null) {
          _window.document.execCommand(
            'Save',
            true,
            fileName || urlConstant.ContractorTraining.Documents + FinalUrl
          );
        }
      } else {
        if (_window != null) {
  ...
  InputMaxwordLength(text: any, maxWords: number) {
    ...
    if (words.length >= maxWords + 1) {
      ...
      if (this.objContractor.trainingProgrammewillfurtheryourcareer) {
        this.objContractor.trainingProgrammewillfurtheryourcareer =
          words.join(' ');
      }
      (
        document.getElementById(
          'TrainingProgrammewillfurtheryourcareer'
        ) as HTMLElement
      ).setAttribute('maxLength', String(Number(words.join(' ').length) - 1));
  ...
  InputResearchundertakenMaxwordLength(text: any, maxWords: number) {
    ...
    if (words.length >= maxWords + 1) {
      ...
      (
        document.getElementById('Researchundertakenifany') as HTMLElement
      ).setAttribute('maxLength', String(Number(words.join(' ').length) - 1));
      return words.join(' ').trim();
    } else {
      if (
  ...
  getFilteredData() {
    ...
    if (this.objFilter.ProgramName) {
      temp = temp.filter(
        (r) => r.programApplyingId == this.objFilter.ProgramName
      );
    }
    if (this.objFilter.ContractorName) {
      temp = temp.filter(
        (r) => r.contractorId == this.objFilter.ContractorName
      );
    }
    ...
    if (this.objFilter.Classification) {
      temp = temp.filter(
        (r) => r.countryClass == this.objFilter.Classification
      );
    }
  ...
  OpenApplicantRanking(ContractorId: any) {
    const url = this.router.serializeUrl(
      this.router.createUrlTree([
        '/admin/ApplicantFinalRanking/' + ContractorId,
      ])
    );
    window.open(url, '_blank');
  }
